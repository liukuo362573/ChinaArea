    ...
    public class AreaHelper
    {
        ...
        public static List<Area> ProcessArea(List<Area> areaList)
        {
            ...
            for (int i = 0; i < provinceList.Count; i++)
            {
                ...
                includeList.Clear();
                Console.WriteLine("获取" + string.Format("[{0} {1}/{2}]", province.AreaName, i + 1, provinceList.Count));
        ...
        {
            Console.WriteLine("获取" + string.Format("[{0} {1}/{2}] {3}", province.AreaName, index + 1, total, city.AreaName));
